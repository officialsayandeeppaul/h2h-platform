...
const ShimmerButton = dynamic(
  () => import("@/components/ui/magic-components").then((mod) => ({ default: mod.ShimmerButton })),
  { ssr: false }
);
const Meteors = dynamic(
  () => import("@/components/ui/magic-components").then((mod) => ({ default: mod.Meteors })),
  { ssr: false }
);
const Particles = dynamic(
  () => import("@/components/ui/magic-components").then((mod) => ({ default: mod.Particles })),
  { ssr: false }
);
...
export function LocationsSection() {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      ([entry]) => setEffectsOn(entry.isIntersecting),
  ...
  return (
    <section ref={ref} className="relative py-28 bg-gray-950 overflow-hidden">
      {effectsOn && (
        <>
          <Meteors number={10} />
          <Particles quantity={18} color="#3b82f6" />
        </>
      )}

      <div className="absolute top-20 left-20 w-72 h-72 bg-blue-600/15 rounded-full blur-[60px] pointer-events-none" />
      <div className="absolute bottom-20 right-20 w-72 h-72 bg-cyan-600/15 rounded-full blur-[60px] pointer-events-none" />
      ...
      <div className="max-w-[1200px] mx-auto px-6 lg:px-12 relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <div className="flex flex-wrap gap-3 mb-10">
              {cities.map((city) => (
                <span
                  key={city}
                  className="px-4 py-2 bg-gray-800/50 backdrop-blur-sm rounded-full border border-gray-700 text-[13px] font-medium text-gray-300 hover:border-cyan-500/50 hover:text-cyan-400 transition-colors duration-150"
                >
                  {city}
                </span>
              ))}
          ...
          <div className="relative h-[500px] flex items-center justify-center">
            <div className="absolute w-[400px] h-[400px] rounded-full border border-blue-500/20" />
            <div className="absolute w-[350px] h-[350px] rounded-full border border-cyan-500/20" />
            <div className="absolute w-[300px] h-[300px] rounded-full border border-teal-500/20" />

            <div className="relative w-[280px] h-[280px]">
              <div
                className="absolute inset-0 rounded-full bg-gradient-to-br from-blue-600/40 via-cyan-500/30 to-teal-500/40"
                style={effectsOn ? { animation: 'spin 20s linear infinite' } : undefined}
              />
